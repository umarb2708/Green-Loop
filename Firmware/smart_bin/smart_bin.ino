...
#define BUTTON_START 0   // Start/Continue button (BOOT button on ESP32-CAM)
#define BUTTON_STOP 16   // Stop/Done button (changed from 2)

#define HX711_DOUT 3  // Changed from 16
#define HX711_SCK 1   // Changed from 17

// I2C pins - MUST NOT be 21/22 on ESP32-CAM (camera conflict)
#define I2C_SDA 12  // Changed from 21 - safe to use with camera
#define I2C_SCL 13  // Changed from 22 - safe to use with camera
...
void setup() {
  ...
  // Check PSRAM
  if(!psramFound()) {
    Serial.println("ERROR: PSRAM not found! Quirc needs PSRAM.");
  } else {
    Serial.println("PSRAM found: OK");
  }
  
  // Initialize I2C
  // Using GPIO 12 (SDA) and GPIO 13 (SCL) - safe pins that don't conflict with camera
  Serial.println("Initializing I2C on GPIO 12/13...");
  ...
  // Try to detect PCA9685 on I2C
  Wire.beginTransmission(0x40);
  if(Wire.endTransmission() == 0) {
    servo_available = true;
    pwm.begin();
    pwm.setPWMFreq(60);
    Serial.println("Servo driver initialized: OK");
    
    // Close all lids initially
    Serial.println("Setting servos to closed position...");
    for(int i = 0; i < NUM_BINS; i++) {
      setServo(i, false);
    }
    Serial.println("Servos set: OK");
  } else {
  ...
  bool load_cell_ready = false;
  
  while(millis() - timeout_start < 2000) { // 2 second timeout
    if(scale.is_ready()) {
      scale.set_scale(420.0983); // Calibration factor - adjust as needed
      scale.tare();
      load_cell_ready = true;
      load_cell_available = true;
      Serial.println("Load cell initialized: OK");
      break;
    }
    delay(100);
  }
  ...
  Serial.println("Preferences loaded: OK");
...
void Wifi_Setup() {
  ...
  Serial.println("Config Mode - OLED not available");
...
// ==================== Servo Control ====================
void setServo(int servo_num, bool open) {
  if(!servo_available) return; // Skip if servo not connected
...
void disposal() {
  ...
  while(continue_flag) {
    ...
    Serial.println("Type: " + plastic_type);
    ...
    if(abs(current_weight - previous_weight) > 5) { // 5g minimum weight change
      ...
      Serial.println("Success! Points: +" + String(points));
...
void displayMessage(String message) {
  if(!oled_available) return; // Skip if OLED not connected
